...
void Execute_Queries(metadata * md, work_line * wl_ptr,uint64_t query)
{
    ...
    uint64_t null_flag_2 = 0;
    uint64_t pos = TAG;
    uint64_t filters_flag_counter = 0;
    // For every Query
    for (uint64_t i = query; i < query + 1; i++)
    {
        ...
        // This case will be TRUE , when left part of first predicate has been found in filters , whose result of filter is 0
        if(null_flag == 1)
        {
            // printf("%lu)",i);
        ...
        // This case will be TRUE , when right part of first predicate has been found in filters , whose result of filter is 0
        else if(null_flag_2 == 1)
        {
            // printf("%lu)",i);
            for (uint64_t z = 0; z < wl_ptr -> selects[i].num_tuples; z++) // for every select
            {
